...
public class BSFSnowGolemRankScreen extends Screen {
    ...
    private static final int MIDDLE_HEIGHT = 25 * 3;  // 3娑擃亣绶崗銉︻攱
    ...
    protected void init() {
        ...
        ScreenAlignment.alignRight(moneyLabel, labelBeginX, labelWidth);
        ScreenAlignment.alignRight(lifespanLabel, labelBeginX, labelWidth);
    ...
    public void onClose() {
        ...
        if (minecraft.level.getEntity(id) instanceof BSFSnowGolemEntity) {
            try {
                ClientPacketDistributor.sendToServer(new UpdateGolemRankPayload(
                        id,
