...
public class StoneSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    ...
    public AbstractBSFSnowballEntity getCorrespondingEntity(Level level, LivingEntity livingEntity, ILaunchAdjustment launchAdjustment, RegionData region) {
        return new StoneSnowballEntity(livingEntity, level, launchAdjustment, region);
    }
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        StoneSnowballEntity snowball = new StoneSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegister.REGION));
    ...
    public void addMainTips(List<Component> pTooltipComponents) {
        pTooltipComponents.add(Component.translatable("stone_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
