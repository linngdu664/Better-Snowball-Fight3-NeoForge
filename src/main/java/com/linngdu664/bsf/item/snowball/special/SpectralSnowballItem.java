...
public class SpectralSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    ...
    public AbstractBSFSnowballEntity getCorrespondingEntity(Level level, LivingEntity livingEntity, ILaunchAdjustment launchAdjustment, RegionData region) {
        return new SpectralSnowballEntity(livingEntity, level, launchAdjustment, region);
    }
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        SpectralSnowballEntity snowball = new SpectralSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegister.REGION));
