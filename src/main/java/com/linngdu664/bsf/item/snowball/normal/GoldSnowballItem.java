...
public class GoldSnowballItem extends AbstractBSFSnowballItem implements ProjectileItem {
    ...
    public AbstractBSFSnowballEntity getCorrespondingEntity(Level level, LivingEntity livingEntity, ILaunchAdjustment launchAdjustment, RegionData region) {
        return new GoldSnowballEntity(livingEntity, level, launchAdjustment, region);
    }
    ...
    public @NotNull Projectile asProjectile(@NotNull Level level, @NotNull Position position, @NotNull ItemStack itemStack, @NotNull Direction direction) {
        GoldSnowballEntity snowball = new GoldSnowballEntity(level, position.x(), position.y(), position.z(), itemStack.get(DataComponentRegister.REGION));
    ...
    public void addMainTips(List<Component> pTooltipComponents) {
        pTooltipComponents.add(Component.translatable("gold_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
