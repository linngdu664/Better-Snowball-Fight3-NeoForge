...
public class ProjectileRepulsionSnowballItem extends AbstractBSFSnowballItem {
    ...
    public @NotNull InteractionResult use(@NotNull Level pLevel, Player pPlayer, @NotNull InteractionHand pUsedHand) {
        ...
        if (pPlayer.isShiftKeyDown()) {
            ...
            if (!pLevel.isClientSide()) {
                ProjectileRepulsionSnowballEntity snowballEntity = new ProjectileRepulsionSnowballEntity(pPlayer, pLevel, getLaunchAdjustment(1), itemStack.get(DataComponentRegister.REGION.get()));
            ...
            if (!pPlayer.getAbilities().instabuild) {
                itemStack.shrink(1);
                pPlayer.getCooldowns().addCooldown(itemStack, 40);
            }
        }
    ...
    public void addMainTips(List<Component> pTooltipComponents) {
        pTooltipComponents.add(Component.translatable("projectile_repulsion_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
