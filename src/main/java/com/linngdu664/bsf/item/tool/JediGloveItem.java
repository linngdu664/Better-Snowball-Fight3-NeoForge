...
public class JediGloveItem extends GloveItem {
    ...
    public void onUseTick(@NotNull Level pLevel, @NotNull LivingEntity pLivingEntity, @NotNull ItemStack pStack, int pRemainingUseDuration) {
        if (!pLevel.isClientSide() && pLivingEntity instanceof Player player) {
            ...
            for (AbstractBSFSnowballEntity snowball : list) {
                ...
                ((ServerLevel) pLevel).sendParticles(PowerParticleOption.create(ParticleTypes.DRAGON_BREATH, 1.0F), snowball.getX(), snowball.getY(), snowball.getZ(), 8, 0, 0, 0, 0.05);
                snowball.discard();
            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.accept(Component.translatable("jedi_glove.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
